...
void allocate_memory(list_t * freelist, list_t * alloclist, int pid, int blocksize, int policy) {
    ...
    */
    int idx_found;
  idx_found = list_get_index_of_by_Size(freelist, blocksize);

  // if no block with size >= blocksize was found, print error, end function
  if (idx_found == -1) {
    printf("Error: Memory Allocation %d blocks\n", blocksize);
    return;
  }

  block_t *newBlock = list_remove_at_index(freelist, idx_found);
  int old_end = newBlock->end;
  newBlock->pid = pid;
  newBlock->end = newBlock->start + blocksize - 1;
  list_add_ascending_by_address(alloclist, newBlock);

  block_t *fragment;
  fragment = malloc(sizeof(block_t));
  fragment->pid = 0;
  fragment->start = newBlock->end + 1;
  fragment->end = old_end;

  // adding fragment to freelist based on specified policy
  if (policy == 1) { // FIFO
...
void deallocate_memory(list_t * alloclist, list_t * freelist, int pid, int policy) { 
    ...
    */
    int idx_found;
  idx_found = list_get_index_of_by_Pid(alloclist, pid);
  ...
  block_t *newBlock = list_remove_at_index(alloclist, idx_found);
  newBlock->pid = 0;

  if (policy == 1) { // FIFO
    list_add_to_back(freelist, newBlock);
  } else if (policy == 2) { // BESTFIT
    list_add_ascending_by_blocksize(freelist, newBlock);
  } else if (policy == 3) { // WORSTFIT
    list_add_descending_by_blocksize(freelist, newBlock);
  }
}
